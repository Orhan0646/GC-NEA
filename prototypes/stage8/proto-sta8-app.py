...
Stage 8: Student Registration, Password Change, Printable Report
Author: Student
"""
...
def leaderboard():
    ...
    return render_template(
        "leaderboard.html",
        overall=overall, topic_data=topic_data, topics=TOPICS,
        current_user_id=current_user_id, role=session.get("role")
    )
...
# ═══════════════════════════════════════════════════════════════════
# ─── STAGE 8: Student Registration, Password Change, Report ─────────
# ═══════════════════════════════════════════════════════════════════

# ─── Feature 1: Register a new student (teacher only) ────────────────
@app.route("/register-student", methods=["GET", "POST"])
def register_student():
    """
    GET:  Show the student registration form.
    POST: Validate and insert the new student into the database.
          Auto-generates a password from the student's first name.
    """
    if not login_required("teacher"):
        return redirect(url_for("teacher_login"))
    ...
    error    = None
    success  = None
    gen_password = None   # shown to teacher after registration
...
# ─── Feature 2: Change password (student only) ───────────────────────
...
# ─── Feature 3: Printable student report (teacher only) ──────────────
@app.route("/student-report/<int:student_id>")
def student_report(student_id):
    """
    Generates a clean printable report page for a specific student.
    Teacher only. Separate from student-history so it has print-optimised CSS.
    """
    if not login_required("teacher"):
        return redirect(url_for("teacher_login"))
