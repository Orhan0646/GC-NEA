...
Stage 7: Flashcard Revision Mode
Author: Student
"""
...
def flashcards():
    """
    Flashcard revision page. Student selects topic and difficulty
    via query parameters. Cards are randomised on each visit.
    GET /flashcards                         → shows topic/difficulty picker
    GET /flashcards?topic=...&difficulty=... → loads cards for that selection
    """
    if not login_required("student"):
        return redirect(url_for("student_login"))
    ...
    # No selection yet — show the picker screen
    ...
    # Fetch questions randomised — ORDER BY RANDOM() shuffles each visit
    cursor.execute("""
    ...
    # Build card list — include the full answer text not just the letter
    cards = []
    for row in rows:
        letter = row["correct_answer"]          # "A", "B", "C" or "D"
        option_map = {
            "A": row["option_a"],
            "B": row["option_b"],
            "C": row["option_c"],
